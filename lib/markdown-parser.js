...
var RecipeDAO_1 = require("./RecipeDAO");
...
function parseMarkdownToJson(markdown, fileName) {
            ...
            for (var i = 0; i < fmLines.length; i++) {
                ...
                if (/^Kategorie\s*:/i.test(l)) {
                    for (var j = i + 1; j < fmLines.length; j++) {
                        ...
                        if (m)
                            categories.push(new RecipeDAO_1.Category(m[1].trim()));
                        else
                            break;
                    }
                    break;
    ...
    if (tableStart !== -1) {
        ...
        dataLines.forEach(function (l) {
            ...
            if (parts.length >= 2) {
                ...
                ingredients.push(new RecipeDAO_1.Ingredient(ingredient_name.trim(), undefined, amount !== null && amount !== void 0 ? amount : undefined, unit ? unit.trim() : unit || undefined, optionalFlag));
            }
        });
    }
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        var m = l.match(/^(\d+)\.\s*(.*)$/);
        if (m) {
            steps.push(new RecipeDAO_1.Step(Number(m[1]), m[2].trim()));
        }
    }
    ...
    var recipe = new RecipeDAO_1.Recipe(title, ingredients, undefined, recipe_description || undefined, steps, categories);
    return recipe;
}
